...
export async function renderSummary(
  ...
  try {
    ...
    if (isYjkEngine) {
      ...
      promptParts.push(
        localize(
          ...
          '本次分析使用了盈建科（YJK）引擎。YJK 已完成整体计算，计算结果保存在 YJK 工程中。'
            ...
            + (yjkProject ? `\nYJK 工程路径：${yjkProject}` : '')
            + (workDir ? `\n工作目录：${workDir}` : ''),
          'This analysis used the YJK engine. YJK has completed the full design calculation and results are saved in the YJK project. '
      ...
      promptParts.push(
        localize(locale, '请用中文在 200 字以内回复。', 'Respond in English within 200 words.'),
      );
    } else {
      promptParts.push(
        hasData
