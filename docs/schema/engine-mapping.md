...
```
API request (analysis_type, model, parameters, engineId?)
  │
  ├─ engineId specified? → validate engine exists & available → use it
  │
  └─ auto-select → filter available engines by capability & model family
...
1. Registry selects engine → resolves matching skill via `intent.md` frontmatter
2. Imports the skill's `runtime.py` module
3. Calls `run_analysis(model, parameters)`
4. On success → injects engine `meta` into the result dict → returns to API
5. On exception → attempts fallback (if auto-selected & fallback engine exists)
6. On fallback success → returns fallback result with `selectionMode: "fallback"`
7. On fallback failure (or no fallback) → re-raises exception to API layer

### Result structure (`AnalysisResult`)

| Field | Type | Description |
|-------|------|-------------|
| `status` | `"success"` \| `"partial"` | Computation outcome |
| `summary` | `dict` | Key metrics (max displacement, base shear, etc.) |
| `detailed` | `dict` | Full result data (node displacements, element forces) |
| `warnings` | `list[str]` | Non-fatal issues encountered during analysis |
| `meta` | `dict` | Engine metadata (injected by registry, not by skill) |

### Error handling contract

| Scenario | Skill must | Registry behavior |
|----------|-----------|-------------------|
| Analysis succeeds | Return `AnalysisResult(status="success")` | Inject meta, return to API |
| Partial result | Return `AnalysisResult(status="partial")` | Inject meta, return to API |
| Analysis fails | `raise RuntimeError(message)` | Try fallback, re-raise if none |
| Engine missing | `raise EngineNotAvailableError(engine, reason)` | Try fallback, re-raise if none |

> **Important**: Skills must never return an error dict. The registry's fallback
> mechanism relies on exceptions to detect failures. Returning `{status: "error"}`
> bypasses fallback and the API would incorrectly report success.

### Engine availability checks

| Engine | Check method | Environment variable |
|--------|-------------|---------------------|
...
| `builtin-yjk` | `YJK_PATH` exists and points to valid directory | `YJK_PATH` |
